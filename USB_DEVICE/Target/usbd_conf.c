...
{
  static uint32_t mem[sizeof(USBD_CDC_HandleTypeDef) + sizeof(USBD_CDC_2_HandleTypeDef) + sizeof(uint32_t)];
  void *alloc = NULL;
  if ((int)p == (int)&USBD_CDC)
  {
    alloc = (void*)&mem[0];
  }
  else if ((int)p == (int)&USBD_CDC_2)
  {
    alloc = (void*)&mem[(sizeof(USBD_CDC_2_HandleTypeDef) + sizeof(uint32_t)) / sizeof(uint32_t)];
  }
  return alloc;
}
#endif
